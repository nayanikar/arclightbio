...
export function snapshotFromOpportunity(
  ...
  return {
    ...
    hypothesis_statement:
      obj.hypothesis?.statement?.trim() || obj.search_query?.trim() || "Discovery program",
    confidence_score: obj.confidence_score ?? 0,
    actionability_zone: obj.actionability_zone ?? "too_early",
    status: obj.status ?? "initialising",
    evidence_card_count: obj.evidence_cards?.length ?? 0,
    challenge_count: obj.challenges?.length ?? 0,
